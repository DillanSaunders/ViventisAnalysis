...
class BdvBase:
    ...
    def _write_pyramids_header(self):
        ...
        for isetup in range(self.nsetups):
            ...
            with h5py.File(self.filename_h5, 'a') as _file_object_h5:
                if group_name in _file_object_h5:
                    grp = _file_object_h5[group_name]
    ...
    def create_pyramids(self, subsamp=((4, 8, 8),), blockdim=((8, 128, 128),), compression=None) -> None:
        ...
        #commented to allow for other compression types
        ...
        for time in trange(self.ntimes, desc='time points'):
            for isetup in trange(self.nsetups, desc='views'):
                for ilevel in range(1, self.nlevels):
                    ...
                    with h5py.File(self.filename_h5, 'a') as _file_object_h5:
                        if full_res_group_name in _file_object_h5:
                            raw_data = _file_object_h5[full_res_group_name]['cells'][()].astype('uint16')
                            pyramid_group_name = self._fmt.format(time, isetup, ilevel)
                            grp = _file_object_h5.create_group(pyramid_group_name)
                            ...
                            grp.create_dataset('cells', data=subdata, chunks=tuple(self.chunks[ilevel]),
                                            maxshape=(None, None, None), compression=self.compression, dtype=self.bit_depth) #'int16'
...
class BdvWriter(BdvBase):
    ...
    def __init__(self, filename,
                 ...
                 overwrite=False,bit_depth = 'int16'):
    ...
    def _write_setups_header(self):
        ...
        for isetup in range(self.nsetups):
            ...
            with h5py.File(self.filename_h5, 'a') as _file_object_h5:
                if group_name in _file_object_h5:
                    del _file_object_h5[group_name]
                grp = _file_object_h5.create_group(group_name)
    ...
    def append_plane(self, plane, z, time=0, illumination=0, channel=0, tile=0, angle=0):
        ...
        with h5py.File(self.filename_h5, 'a') as _file_object_h5:
            for ilevel in range(self.nlevels):
                ...
                dataset = _file_object_h5[group_name]["cells"]
            ...
            for ilevel in range(self.nlevels):
                ...
                if stack is not None:
                    ...
                    grp.create_dataset('cells', chunks=self.chunks[ilevel],
                                    ...
                                    compression=self.compression, dtype=self.bit_depth)#'int16'
    ...
    def write_xml(self, camera_name="default",  microscope_name="default",
        ...
        #Dillan added
        ET.SubElement(tpoints,'interval').text = self.timestep
        ET.SubElement(tpoints,'unit').text = self.time_unit
